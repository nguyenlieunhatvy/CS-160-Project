...
static void run (face_landmark_node *face_landmark_list_head, char* fileName)
{
    ...
    if (input_filename[0] != '\0')
    {
        ...
        cv::split(cv::imread (&input_filename[0]), rgbChannels);
        source_image = rgbChannels[2];
        if (!source_image.empty())
        {
            ...
            float x_cor;
            float y_cor;
            while (counter < NUM_LANDMARKS - 2 && face_landmark_element != NULL)
            {
                x_cor = face_landmark_element->x;
                y_cor = face_landmark_element->y;
                

                if(x_cor >= source_image_resolution.width)
                    x_cor = source_image_resolution.width - 1;
                if(y_cor >= source_image_resolution.height)
                    y_cor = source_image_resolution.height - 1;

                ++counter;
                subdiv.insert(cv::Point2f(x_cor, y_cor));
            ...
            circle(source_image, cv::Point2f(face_landmark_element->x, face_landmark_element->y), 5, 1234, 4);
            face_landmark_element = face_landmark_element->next;
            circle(source_image, cv::Point2f(face_landmark_element->x, face_landmark_element->y), 5, 1234, 4);

            //scout << "writing to " << output_filename << endl;
            cv::merge(rgbChannels, source_image);
